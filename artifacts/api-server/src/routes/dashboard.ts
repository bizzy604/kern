...
import { workSessionsTable, standupsTable } from "@workspace/db";
...
router.get("/dashboard/summary", requireSession, async (req, res) => {
  try {
    ...
    const [todaySessions, weeklySessions, todayStandup, recentSessions] = await Promise.all([
      db.select().from(workSessionsTable).where(
        and(eq(workSessionsTable.developerId, devId), gte(workSessionsTable.startedAt, todayStart), lte(workSessionsTable.startedAt, todayEnd)),
      ),
      db.select().from(workSessionsTable).where(
        and(eq(workSessionsTable.developerId, devId), gte(workSessionsTable.startedAt, weekStart)),
      ),
      db.select().from(standupsTable).where(
        and(eq(standupsTable.developerId, devId), eq(standupsTable.date, today))
      ).limit(1),
      db.select().from(workSessionsTable).where(eq(workSessionsTable.developerId, devId))
        .orderBy(desc(workSessionsTable.startedAt)).limit(5),
    ]);
    ...
    const topActivityToday = Object.keys(typeCounts).length > 0
...
router.get("/dashboard/activity-breakdown", requireSession, async (req, res) => {
  try {
    ...
    for (let i = 0; i < days; i++) {
      ...
      dailyMap[d.toISOString().split("T")[0]] = { totalMinutes: 0, sessionCount: 0, breakdown: {} };
    }
    for (const s of sessions) {
      ...
      if (dailyMap[dateStr]) {
        ...
        dailyMap[dateStr].breakdown[s.activityType] = (dailyMap[dateStr].breakdown[s.activityType] || 0) + s.durationMinutes;
      }
    }

    return res.json({ days, breakdown, dailySeries: Object.entries(dailyMap).map(([date, data]) => ({ date, ...data })) });
  } catch (err) {
