...
import { workSessionsTable } from "@workspace/db";
...
router.get("/sessions", requireSession, async (req, res) => {
  try {
    ...
    const [sessions, total] = await Promise.all([
      db.select().from(workSessionsTable).where(and(...conditions))
        .orderBy(desc(workSessionsTable.startedAt)).limit(limit).offset(offset),
      db.select({ id: workSessionsTable.id }).from(workSessionsTable).where(and(...conditions)),
    ]);

    return res.json({
      sessions: sessions.map(s => ({ ...s, startedAt: s.startedAt.toISOString(), endedAt: s.endedAt.toISOString() })),
      total: total.length,
    });
  } catch (err) {
...
router.get("/sessions/:id", requireSession, async (req, res) => {
  try {
    const parsed = GetSessionParams.safeParse({ id: req.params.id });
    if (!parsed.success) return res.status(400).json({ error: "Invalid id" });

    const session = await db.select().from(workSessionsTable)
      .where(eq(workSessionsTable.id, parsed.data.id)).limit(1);

    if (!session[0]) return res.status(404).json({ error: "Session not found" });

    const s = session[0];
    return res.json({ ...s, startedAt: s.startedAt.toISOString(), endedAt: s.endedAt.toISOString() });
  } catch (err) {
